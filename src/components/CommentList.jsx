            ...
            {this.props.commentList.map((comment, i) => (
              <React.Fragment key={i}>
                <Card className={classes.card}>
                  <CardHeader
                    avatar={
                      comment.author[0].gd$image.src === 'https://img1.blogblog.com/img/b16-rounded.gif' ? (
                        <Avatar aria-label="photos" className={classes.avatar}>
                          {comment.author[0].name.$t[0]}
                        </Avatar>
                      ) : (
                        <Avatar aria-label="photos" src={comment.author[0].gd$image.src} className={classes.avatarTransparent}></Avatar>
                      )
                    }
                    action={
                      <IconButton
                  ...
                  <Card className={classes.cardReply}>
                    <CardContent>
                      <iframe
                        ...
                        frameBorder="0"
                        src={'//www.blogger.com/comment-iframe.g?blogID=' + this.props.blogID + '&postID=' + this.props.postID + '&parentID=' + comment.id + '&skin=contempo'}
                        width="100%"
                ...
                {this.props.commentReplyList
                  ...
                  .map((reply) => (
                    <Card className={classes.cardReply} key={reply.id}>
                      <CardHeader
                        avatar={
                          reply.author[0].gd$image.src === 'https://img1.blogblog.com/img/b16-rounded.gif' ? (
                            <Avatar aria-label="photos" className={classes.avatar}>
                              {reply.author[0].name.$t[0]}
                            </Avatar>
                          ) : (
                            <Avatar aria-label="photos" src={reply.author[0].gd$image.src} className={classes.avatarTransparent}></Avatar>
                          )
                        }
