...
class PostPage extends Component {
  ...
  fetchCommentList() {
    axios({
      url: `${process.env.NODE_ENV === 'development' ? '//usereact.blogspot.com' : ''}/feeds/${this.state.post.id.$t
        .split('-')
        .pop()}/comments/default?alt=json-in-script&reverse=false&orderby=published&start-index=1`,
      adapter: jsonpAdapter,
      callbackParamName: 'c',
    }).then((res) => {
      if (res.data.feed.entry) {
  ...
  render() {
    ...
    return (
      <Grid item xs={12} sm={8} className={classes.box}>
          ...
          <Box p={2}>
            {this.state.isFetch ? (
              <Breadcrumbs separator={<NavigateNextIcon fontSize="small" />} aria-label="breadcrumb" maxItems={2}>
                ...
                {this.state.post.category.length ? (
                  <Link color="inherit" href={'/search/label/' + this.state.post.category[0].term}>
                    {this.state.post.category[0].term}
                  </Link>
                ) : null}
        ...
        <Card className={classes.card}>
          {!this.state.isFetch ? (
            ...
            <React.Fragment>
              ...
              <CardHeader
                avatar={
                  this.state.post.author[0].gd$image.src === 'https://img1.blogblog.com/img/b16-rounded.gif' ? (
                    <Avatar aria-label="photos" className={classes.avatar}>
                      {this.state.post.author[0].name.$t[0]}
                    </Avatar>
                  ) : (
                    <Avatar aria-label="photos" src={this.state.post.author[0].gd$image.src} className={classes.avatarTransparent}></Avatar>
                  )
                }
                action={
                  <React.Fragment>
                    ...
                    <IconButton
                      ...
                      aria-haspopup="true"
                      onClick={(event) => {
                        const url = this.state.post.link
                          .filter((arr) => arr.rel === 'alternate')
                          .map((arr) => arr.href)
                          .toString()
                        this.setState({
                ...
                subheader={
                  <Typography variant="caption" display="block" className={classes.dateClass}>
                    <TimeAgo date={this.state.post.published.$t} />
                  </Typography>
                }
              />
              <Divider />
          ...
          <React.Fragment>
            {!this.state.commentButtonHide ? (
              <Box textAlign="center" mb={2}>
                ...
                >
                  Load {this.state.post.thr$total.$t + ' '}Comment
                  {this.state.post.thr$total.$t > 1 ? 's' : null}
                </Button>
              </Box>
            ) : (
              <CommentList
                commentList={this.state.commentList}
                commentReplyList={this.state.commentReplyList}
                totalComment={this.state.post.thr$total.$t}
                isFetchComment={this.state.isFetchComment}
                blogID={/blog-?(\d+)\./.exec(this.state.post.id.$t)[1]}
                postID={this.state.post.id.$t.split('-').pop()}
                newReply={this.newReply}
                isComment={this.state.newComment}
          ...
          <React.Fragment>
            {this.state.newComment ? (
              <Card>
                <CardContent>
                  <iframe
                    title="iframe-comment"
                    frameBorder="0"
                    src={'//www.blogger.com/comment-iframe.g?blogID=' + /blog-?(\d+)\./.exec(this.state.post.id.$t)[1] + '&postID=' + this.state.post.id.$t.split('-').pop() + '&skin=contempo'}
                    width="100%"
        ...
        >
          <Link color="inherit" component="a" underline="none" href={'//twitter.com/share?url=' + this.state.shareURL} target="_blank" rel="noreferrer">
            <MenuItem>Twitter</MenuItem>
          </Link>
          <Link color="inherit" component="a" underline="none" href={'//www.facebook.com/sharer/sharer.php?u=' + this.state.shareURL} target="_blank" rel="noreferrer">
