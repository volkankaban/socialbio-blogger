...
class PostList extends Component {
  ...
  loadMore(event) {
    ...
    const query = this.props.isSearchPage && qs.parse(this.props.location.search, { ignoreQueryPrefix: true }).q ? '&q=' + qs.parse(this.props.location.search, { ignoreQueryPrefix: true }).q : ''
    axios({
      url: `${process.env.NODE_ENV === 'development' ? '//usereact.blogspot.com' : ''}/${url}?orderby=published&start-index=${
        onPage * listPerPage + 1
      }&max-results=${listPerPage}&alt=json-in-script${query}`,
      adapter: jsonpAdapter,
      callbackParamName: 'c',
    }).then((res) => {
      if (res.data.feed.entry) {
              ...
              <Grid item xs={12} sm={4} key={i} className={classes.box}>
                <Card>
                  ...
                  <CardHeader
                    ...
                    title={<Skeleton height={16} className={classes.skeletoSubHeader} />}
                    subheader={<Skeleton height={11} width="40%" className={classes.skeletoSubHeader} />}
                  />
                </Card>
              </Grid>
            ))
              ...
              <CardContent>
                <Typography variant="h5" component="h2">
                  <Link
                    ...
                    component="a"
                  >
                    <LinesEllipsis text={post.title.$t} maxLine="2" ellipsis="..." trimRight basedOn="letters" />
                  </Link>
                </Typography>
              </CardContent>
              {post.media$thumbnail ? (
                <CardMedia className={classes.media} image={post.media$thumbnail.url.replace('/s72-c/', '/s240/').replace(/^(https?:|)/, '')} title={post.title.$t} />
              ) : (
                <Skeleton variant="rect" className={classes.media} />
              )}
              <CardHeader
                avatar={
                  post.author[0].gd$image.src === 'https://img1.blogblog.com/img/b16-rounded.gif' ? (
                    <Avatar aria-label="photos" className={classes.avatar}>
                      {post.author[0].name.$t[0]}
                    </Avatar>
                  ) : (
                    <Avatar aria-label="photos" src={post.author[0].gd$image.src} className={classes.avatarTransparent}></Avatar>
                  )
                }
                action={
                  <React.Fragment>
          ...
          <Grid item xs={12} className={classes.box}>
            <Box textAlign="center">
              <Button
                ...
                size="large"
                color="primary"
                disabled={this.state.loadingBar}
                onClick={(e) => {
                  this.loadMore()
                }}
              >
                {this.state.loadingBar ? <CircularProgress size={30} thickness={5} className={classes.loadingMargin} /> : null}Load More
              </Button>
            </Box>
          </Grid>
        )}
